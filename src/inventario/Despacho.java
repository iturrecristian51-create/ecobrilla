...
public class Despacho implements Serializable {
    ...
    // Fecha: compatibilidad en String e LocalDateTime
    public LocalDateTime getFechaHora() { return fechaHora; }
    public void setFechaHora(LocalDateTime fechaHora) { if (fechaHora != null) this.fechaHora = fechaHora; }

    // setter que acepta String ISO (por compatibilidad)
    public void setFechaHora(String fechaHoraStr) {
        if (fechaHoraStr == null || fechaHoraStr.isBlank()) return;
        try {
            this.fechaHora = LocalDateTime.parse(fechaHoraStr);
        } catch (DateTimeParseException e) {
            // si no es ISO, intentamos dejar como now (o podríamos intentar parseo custom)
            // para evitar fallos de parseo dejamos valor actual y no lanzamos excepción
        }
    }

    // método que devuelve la fecha como String (para código que espera getFecha())
    public String getFecha() { return fechaHora != null ? fechaHora.toString() : ""; }
