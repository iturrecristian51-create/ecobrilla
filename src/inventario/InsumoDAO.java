...
public class InsumoDAO {
    ...
    public static void registrarHistorialInsumo(HistorialInsumo h) {
        String sql = "INSERT INTO historial_insumos(fecha, lote, nombre, accion, cantidad, observacion) VALUES(?,?,?,?,?,?)";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, h.getFecha());
            ps.setString(2, h.getLote());
            ps.setString(3, h.getNombreInsumo());
            ps.setString(4, h.getAccion());
            ps.setDouble(5, h.getCantidad());
            ps.setString(6, h.getObservacion());
            ps.executeUpdate();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
